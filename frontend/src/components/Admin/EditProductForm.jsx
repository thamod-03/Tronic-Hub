...
const EditProductForm = ({id, product, setEditingProduct}) => {
  const [image, setImage] = useState(null);
  const [name, setName] = useState(product.name);
  const [category, setCategory] = useState(product.category._id);
  const [price, setPrice] = useState(product.oldPrice);
  const [offerPrice, setOfferPrice] = useState(product.newPrice);
  const [stock, setStock] = useState(product.stock);
  const { categories } = useCategory();
  const { capitalizeWords, navigate } = useApp();
  const { updateProductAdmin } = useProduct();
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await updateProductAdmin(id, productData);
    if (result.success) {
